...
! function() {
  ...
  var hb = "",
    ...
    lum = 0,
    xp = 0;
  var hour, minut, ampm_obj, seconds = [];
  ...
  function toc() {
    ...
    var t = new Date(),
      hr = n(t.getHours() > 12 ? t.getHours() - 12 : t.getHours()),
      mn = n(t.getMinutes()),
      sc = n(t.getSeconds()),
      w, h, img, sec;

    var t = new Date(),
      hours_24 = t.getHours(), // <-- NEW
      ampm = hours_24 >= 12 ? 'PM' : 'AM', // <-- NEW
      hours_12 = hours_24 % 12; // <-- NEW
    hours_12 = hours_12 ? hours_12 : 12; // 0 o'clock (midnight) should be 12 // <-- NEW

    var hr = n(hours_12), // <-- MODIFIED
      mn = n(t.getMinutes()),
      sc = n(t.getSeconds()),
      w, h, img, sec;
    ...
    if (mn != mb) {
      ...
      w = (screen.width / 5) | 0;
      PHY2D.delete(minut);
      minut = addNumber(w, screen.width - w * 1.2, mn, 4, 0);
      mb = mn;

      w = (screen.width / 8) | 0; // <-- NEW: Set a size for the AM/PM text
      PHY2D.delete(ampm_obj); // <-- NEW: Delete the old one
      ampmb = ampm; // <-- NEW: Update the tracking variable
    ...
    w = (screen.width / 15) | 0;
    sec = addNumber(w, w + (xp * w) % (screen.width - (w * 2)), sc, 0.05, Math.random() * 3 * Math.PI);
    seconds.push(sec);
    xp++;
  }
